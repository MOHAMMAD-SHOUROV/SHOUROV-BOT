...
try {
  ...
  console.log('✓SHOUROV-BOT Config loaded');
} catch (err) {
  console.error('❌SHOUROV-BOT Failed to load config.json:', err.message);
  process.exit(1);
}
...
global.config.BOTNAME = global.config.BOTNAME || (config && config.BOTNAME) || 'SHOUROV-BOT';
...
try {
  if (fs.existsSync(FBSTATE_PATH)) {
    ...
    console.log('✓SHOUROV-BOT Facebook state (fbstate.json) loaded');
  } else {
    console.warn('⚠ fbstate.json not found — first-time login may require credentials.');
  }
} catch (err) {
  console.error('❌SHOUROV-BOT Error reading fbstate.json:', err.message);
}

// ---------- Start Facebook login & listener ----------
login({ appState }, (err, api) => {
  if (err) {
    console.error('SHOUROV-BOT❌ Facebook login error:', err);
    return;
  }

  console.log('✓SHOUROV-BOT Facebook login successful');
