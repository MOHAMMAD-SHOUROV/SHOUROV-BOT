...
const config = require("../../shourov.json");
...
var apiryukoValue;
try {
  global.client.apishourovPath = join(global.client.mainPath, "../configs/api.json");
  apishourovValue = require(global.client.apishourovPath);
} catch (e) {
  return;
}
try {
  for (const apiKeys in apishourovValue) global.apiryuko[apiKeys] = apiryukoValue[apiKeys];
} catch (e) {
  return;
}
var shourovValue;
try {
  global.client.shourovPath = join(global.client.mainPath, "../configs/shourov.json");
  shourovValue = require(global.client.shourovPath);
} catch (e) {
  return;
}
try {
  for (const Keys in shourovValue) global.shourov[Keys] = shourovValue[Keys];
} catch (e) {
  return;
}
var configValue;
try {
  global.client.configPath = join(global.client.mainPath, "../../shourov.json");
  configValue = require(global.client.configPath);
  logger.loader(`deploying ${chalk.blueBright('shourov')} file`);
} catch (e) {
  return logger.loader(`cant read ${chalk.blueBright('shourov')} file`, "error");
}
try {
  for (const key in configValue) global.config[key] = configValue[key];
  logger.loader(`deployed ${chalk.blueBright('shourov')} file`);
} catch (e) {
  return logger.loader(`can't deploy ${chalk.blueBright('shourov')} file`, "error")
}
...
try {
  if (!global.config.BOTNAME) {
    logger.error(`please enter your bot name in ${chalk.blueBright('shourov.json')} file`);
    process.exit(0);
  }
  if (!global.config.EMAIL) {
    logger.err(`please enter your email address in ${chalk.blueBright('shourov.json')} file`)
    process.exit(0);
  }
  if (!global.config.PREFIX) {
    logger.error(`please enter your bot prefix in ${chalk.blueBright('shourov.json')} file`)
    process.exit(0);
  }
  if (global.config.author != "shourov") {
    logger.error(`detected : author was changed at ${chalk.blueBright('shourov.json')}`);
    process.exit(0);
  }
  if (packages.author != "shourov") {
    logger.error(`detected : author was changed at ${chalk.blueBright('package.json')}`);
    process.exit(0);
  }
  if (packages.name != "shourov") {
...
try {
  var appStateFile = resolve(join(global.client.mainPath, "../../shourovstate.json"));
  ...
  console.log(chalk[`${cnslEvent.logger.colors.cmdStarter}`](`${cnslEvent.logger.strings.cmdStarter}`));
        for (const command of listCommand) {
          try {
            ...
            const configures = require(`../../shourov.json`);
            ...
            if (envConfig) {
              ...
              var ryukoPath = require('../configs/shourov.json');
    ...
    global.custom = require('../../shourov.js')({ api: api });
...
(async () => {
  try {
    ...
    onBot(botData);
  } catch (error) { logger(`can't deploy ${chalk.blueBright('database')} system`, "Shourov") }
})();
