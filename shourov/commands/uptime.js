...
const time = process.uptime() ,
  ...
  const fs = require('fs-extra');
   if (!fs.existsSync(__dirname +
        ...
        fs.writeFileSync(__dirname + `/shourov/UTM-Avo.ttf`, Buffer.from(getfont, "utf-8"));
      }
         if (!fs.existsSync(__dirname +
      ...
      fs.writeFileSync(__dirname + `/shourov/phenomicon.ttf`, Buffer.from(getfont2, "utf-8"));
    };
  if (!fs.existsSync(__dirname +
      ...
      fs.writeFileSync(__dirname + `/shourov/CaviarDreams.ttf`, Buffer.from(getfont3, "utf-8"));
    };
  ...
  const Canvas = require('canvas');
    let pathImg = __dirname + `/shourov/avatar_1111231.png`;
    let pathAva = __dirname + `/shourov/avatar_3dsc11.png`;
...
let l1 = await loadImage(pathAva);
  ...
  ctx.drawImage(l1, 800, -160, 1100, 1100);
     registerFont(__dirname + `/shourov/phenomicon.ttf`, {
      family: "phenomicon"
    });
    ctx.textAlign = "start";
    ...
    ctx.beginPath();
  ////////////////////////////////////////
   registerFont(__dirname + `/shourov/UTM-Avo.ttf`, {
      family: "UTM"
    });
    ctx.textAlign = "start";
    ctx.font = "70px UTM";
    ...
    ctx.restore();
    ctx.save();
registerFont(__dirname + `/shourov/CaviarDreams.ttf`, {
      family: "time"
    });
    ctx.textAlign = "start";
    ctx.font = "45px time";
