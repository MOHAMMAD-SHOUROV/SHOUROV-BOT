...
start: async function({ shourov, events, args, lang }) {
  ...
  if (!args[0]) return shourov.reply(lang("missing"), threadID, messageID);


    let np = args.join(" ");
   if (!args[1]) shourov.reply(lang("wait"), events.threadID, (err, info) => setTimeout(() => { shourov.unsendMessage(info.messageID) }, 20000));
