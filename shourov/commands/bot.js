...
module.exports = {
  ...
  handleReply: async function ({ api, event }) {
    try {
      ...
      const apiData = await axios.get('https://raw.githubusercontent.com/MOHAMMAD-SHOUROV/shourovbot/main/api.json');
      const apiUrl = apiData.data.sim;
      const kl = await axios.get(`https://raw.githubusercontent.com/MOHAMMAD-SHOUROV/shourovbot/main/api.json`);
  ...
  start: async function ({ shourov, events, args, Users }) {
    try {
      const msg = args.join(" ");
      const apiData = await axios.get('https://raw.githubusercontent.com/MOHAMMAD-SHOUROV/shourovbot/main/api.json');
      const apiUrl = apiData.data.sim;
      ...
      if (!msg) {
        ...
        return shourov.reply({
          ...
          if (error) {
            return shourov.reply('An error occurred while processing your request. Please try again later.', events.threadID, events.messageID);
          }
        ...
        if (options.includes(selectedStyle)) {
          ...
          return shourov.reply({ body: `Text type set to "${selectedStyle}" successfully!` }, events.threadID, events.messageID);
        } else {
          return shourov.reply({ body: `Invalid text type! Please choose from: ${options.join(", ")}` }, events.threadID, events.messageID);
        }
      }
        ...
        return shourov.reply({ body: replyMessage }, events.threadID, events.messageID);
      }
        ...
        if (!question) {
          return shourov.reply('Please provide a question to get information about.', events.threadID, events.messageID);
        }
        ...
        if (!answers || answers.length === 0) {
          return shourov.reply(`No information available for the question: "${question}"`, events.threadID, events.messageID);
        }
        ...
        return shourov.reply({ body: helpMessage }, events.threadID, events.messageID);
      } 
        ...
        shourov.reply({ body: styledText }, events.threadID, (error, info) => {
          if (error) {
            return shourov.reply('An error occurred while processing your request. Please try again later.', events.threadID, events.messageID);
          }
      ...
      console.log(error);
      shourov.reply('An error has occurred, please try again later.', events.threadID, events.messageID);
    }
}
}
