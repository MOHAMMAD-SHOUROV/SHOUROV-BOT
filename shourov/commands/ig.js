...
module.exports.config = {
  // IMPORTANT: command 'name' must be a literal string (not a template using global.* at load time)
  name: "islm",
  version: "1.0.0",
  permission: 0,
  credits: "shourov",
...
module.exports.run = async ({ api, event }) => {
  try {
    // prefer global.nodemodule if your loader provides it, otherwise fallback to require
    ...
    // Prepare cache dir and path safely
    const cacheDir = path.join(__dirname, 'cache');
    try {
      if (!fs.existsSync(cacheDir)) fs.mkdirSync(cacheDir, { recursive: true });
    } catch (e) {
      // if fs-extra doesn't have existsSync same signature, fallback
      try { fs.mkdirSync(cacheDir, { recursive: true }); } catch (_) {}
    }

    const filePath = path.join(cacheDir, `islm_pic_${Date.now()}.jpg`);

    // Download image using axios (stream) and save to file
